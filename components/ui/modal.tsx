...
export const Modal: React.FC<ModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-stone-900/60 backdrop-blur-sm animate-fade-in">
      ...
      />
      <div
        className={`relative w-full ${widthStyles[maxWidth]} max-h-[92vh] sm:max-h-[88vh] flex flex-col bg-white rounded-2xl shadow-2xl border border-stone-200 overflow-hidden z-10`}
        role="dialog"
        aria-modal="true"
      >
        {title && (
          <div className="flex items-center justify-between px-5 sm:px-6 py-3.5 sm:py-4 border-b border-stone-100 flex-shrink-0 bg-white">
            ...
            <button
              onClick={onClose}
              className="p-2 text-stone-400 hover:text-stone-700 rounded-xl hover:bg-stone-100 transition-colors flex-shrink-0 active:scale-95"
              aria-label="Kapat"
            >
              <X className="w-5 h-5" />
            </button>
